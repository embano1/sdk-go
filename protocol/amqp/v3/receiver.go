...
func (r *receiver) Receive(ctx context.Context) (binding.Message, error) {
	// In v1.0.5, Receive requires a ReceiveOptions parameter however this is only for future expansion hence omitting
	// from our API
	var receiveOpts amqp.ReceiveOptions

	m, err := r.amqp.Receive(ctx, &receiveOpts)
	if err != nil {
		if err == ctx.Err() {
			return nil, io.EOF
		}

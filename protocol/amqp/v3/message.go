...
func (m *Message) Finish(err error) error {
	ctx := context.TODO() // Use a timeout context if needed in a real implementation
	if err != nil {
		return m.AMQPrcv.RejectMessage(ctx, m.AMQP, &amqp.Error{
			Condition:   condition,
			Description: err.Error(),
		})
	}
	return m.AMQPrcv.AcceptMessage(ctx, m.AMQP)
}
